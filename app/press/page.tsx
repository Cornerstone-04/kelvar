...
export default function PressPage() {
  return (
    <div className="min-h-screen pt-16.25" style={{ background: "var(--bg)" }}>
      {/* Header */}
      <div
      ...
      <div className="px-6 md:px-12 py-20">
        ...
        <div className="space-y-0">
          {press.map((item, i) => (
            ...
            >
              <div className="text-[0.6rem] tracking-[0.2em] uppercase text-white/25 font-mono">
                {item.outlet}
              </div>
              <h3
                className="font-heading font-bold text-white/70 group-hover:text-white transition-colors duration-200"
                style={{ fontSize: "clamp(1rem, 2.5vw, 1.4rem)" }}
              >
                {item.headline}
              </h3>
              <div className="text-[0.6rem] tracking-widest text-white/20 font-mono md:text-right">
                {item.date}
              </div>
            </motion.a>
          ))}
      ...
      >
        ...
        <a
          href={`mailto:${site.email}?subject=Press Enquiry`}
