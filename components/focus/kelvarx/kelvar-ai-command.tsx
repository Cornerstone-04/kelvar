    ...
    >
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[0.8fr_1.2fr]">
        ...
        <div className="border border-white/10 bg-[#08082d] p-4 md:p-7">
          <div className="flex items-center justify-between border-b border-white/10 pb-4 font-mono text-xxs uppercase tracking-[0.18em] text-white/35">
          ...
          <div className="mt-4 grid min-h-80 gap-4 md:grid-cols-[1.25fr_0.75fr]">
            <div className="relative overflow-hidden border border-white/8 bg-[radial-gradient(circle_at_65%_40%,rgba(245,245,249,0.12),transparent_6%),linear-gradient(135deg,rgba(255,255,255,0.04),transparent)]">
              ...
              <p className="absolute bottom-4 left-4 font-mono text-xxs uppercase tracking-[0.18em] text-white/35">
                Atmospheric field / no live data
              </p>
            </div>
            <div className="grid gap-px bg-white/10">
              {["Fleet overview", "Telemetry", "Mission state", "Intelligence events"].map(
                (label) => (
      ...
      <div className="mt-8 grid gap-px bg-white/10 sm:grid-cols-2 lg:grid-cols-4">
        {command.capabilities.map((capability, index) => (
          <div key={capability} className="bg-bg p-5">
            <p className="font-mono text-xxs text-dim">
              {String(index + 1).padStart(2, "0")}
            </p>
            <p className="mt-3 font-heading text-xl font-bold">{capability}</p>
          </div>
        ))}
      </div>
    </section>
